  ...
  return (
    ...
    >
      ...
      {isOpen && (
        ...
        >
          <div className="flex items-center justify-between px-2 pb-1.5 border-b border-slate-100 mb-2">
          ...
          <div className="grid gap-1">
            {categories.map((category) => {
              const catName = category.name || category.categoryTitle;
              const subcats = (category.subcategories || [])
                .map((sub) =>
                  typeof sub === "string" ? sub : sub?.name || sub?.subCategoryTitle
                )
                .filter(Boolean);

              return (
                <div
                  key={category.id || category._id || catName}
                ...
                >
                  ...
                  {subcats.length > 0 && (
                    <div className="ml-3 flex flex-wrap gap-1 pb-1 pl-2 border-l-2 border-[#f0dcf5]">
                      {subcats.map((subTitle) => (
                        <a
                          key={String(subTitle)}
                          href={`#${getCategoryRoute(category, String(subTitle))}`}
                          onClick={close}
                          className="rounded px-2 py-0.5 text-xs text-slate-600 transition hover:bg-white hover:text-[#79259c] hover:font-semibold"
                        >
                          {subTitle}
                        </a>
                      ))}
            ...
            {!categories.length && (
              <p className="px-2 py-2 text-sm text-slate-500">Categories are loading…</p>
            )}
          </div>
        </div>
      )}
