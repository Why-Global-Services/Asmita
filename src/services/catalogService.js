...
const normalizeProduct = (product) => {
  ...
  return {
    ...
    inStock: product.status !== undefined ? product.status : true,
...
const normalizeCategory = (cat, subCategoryMap = {}) => {
  ...
  const dynamicSubcats = subCategoryMap[id] || subCategoryMap[name] || cat.subcategories || [];
  return {
    ...cat,
    id,
    _id: id,
...
export const catalogService = {
  ...
  getProduct: async (id) => {
    ...
    if (Array.isArray(apiData) && apiData.length) {
      ...
      rawProduct = apiData.mainProduct || apiData;
    }
  ...
  getCategories: async () => {
    ...
    if (Array.isArray(subCategoriesRes)) {
      subCategoriesRes.forEach((sub) => {
        const subTitle = sub.subCategoryTitle || sub.name || sub.title;
        if (!subTitle) return;

        if (sub.categoryId) {
          if (!subCategoryMap[sub.categoryId]) subCategoryMap[sub.categoryId] = [];
          if (!subCategoryMap[sub.categoryId].includes(subTitle)) {
            subCategoryMap[sub.categoryId].push(subTitle);
          }
        }
        if (sub.categoryTitle) {
          if (!subCategoryMap[sub.categoryTitle]) subCategoryMap[sub.categoryTitle] = [];
          if (!subCategoryMap[sub.categoryTitle].includes(subTitle)) {
            subCategoryMap[sub.categoryTitle].push(subTitle);
          }
        }
      });
    }
    ...
    let list = Array.isArray(categoriesRes)
      ? categoriesRes.map((cat) => normalizeCategory(cat, subCategoryMap))
      : [];
    return list;
  },
