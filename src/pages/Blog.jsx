...
export default function Blog() {
  ...
  return (
    <>
      ...
      <main className="mx-auto grid max-w-7xl gap-8 px-5 py-10 lg:grid-cols-[1fr_250px] sm:px-8">
        <section>
          <div className="mb-7">
            <p className="text-xs font-bold tracking-widest text-[#79259c]">
              HEALTH INSIGHTS
            </p>
            <h2 className="mt-2 font-serif text-3xl">Latest Articles</h2>
          </div>
          {!items ? (
            <Loader />
          ) : (
            <>
              <article className="mb-7 rounded-xl bg-gradient-to-r from-[#f5e8f8] to-[#e4edf0] p-7">
                ...
                <a
                  className="mt-4 inline-block font-bold text-[#79259c]"
        ...
        <aside className="h-max rounded-xl border border-slate-200 p-5">
          ...
          {[
            ...
            <a className="block border-b border-slate-100 py-3 text-sm" key={x}>
              {x}
            </a>
          ))}
          <h3 className="mt-6 font-bold text-[#79259c]">Latest Posts</h3>
          {items?.slice(0, 2).map((x) => (
            <p className="border-b border-slate-100 py-3 text-sm" key={x.id}>
              <b>{x.title}</b>
